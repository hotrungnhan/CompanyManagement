...
class PerformanceModel (
    @get: PropertyName("basic_point")
    @set: PropertyName("basic_point")
    var BasicPoint: Long = 0,
    ...
    var Late: Long = 0,
    @get: PropertyName("month_over_time")
    @set: PropertyName("month_over_time")
    var MonthOverTime: Long = 0,
    ...
    fun computeBasicPoint() {
        BasicPoint = YearMonth.now().lengthOfMonth().toLong()
    }

    fun computeTotalPoint() = BasicPoint + MonthOverTime * 10 + TaskDone * 2 - Late * 2 - AbsenceA - AbsenceNA * 3
    fun computeTaskBonus() = 50000 * TaskDone
    fun computeOverTimeBonus() = 200000 * MonthOverTime
